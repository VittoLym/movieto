<template>
  <nav class="fixed bottom-0 w-full z-50 bg-surface/90 backdrop-blur-2xl flex justify-around items-center h-20 pb-safe px-4 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] border-t border-outline/5">
    <a 
    ...
    >
      ...
      <span class="font-label-md text-label-md mt-1" v-if="item.active">{{ item.label }}</span>
      <span class="font-label-md text-label-md mt-1" v-else>{{ item.label }}</span>
    </a>
  </nav>
</template>
